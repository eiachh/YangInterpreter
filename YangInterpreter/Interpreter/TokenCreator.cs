    ...
    internal static class TokenCreator
    {
        ...
        internal static void Init()
        {
            InterpreterSearchSchemeList = new List<SearchScheme>
            {
                ...
                //new SearchScheme(new Regex(@"^\s*config (true|false);\s*$"),TokenTypes.ConfigStatement,-1, 1, typeof(Config)),
                //new SearchScheme(new Regex(@"^\s*typedef ([a-z0-9A-Z-]*) {\s*$"),TokenTypes.Typedef,1,-1),
                //new SearchScheme(new Regex("^\\s*range \"([0-9]* ?.. ?[0-9]*)\";\\s*$"),TokenTypes.Range,-1,1, typeof(Range)),
